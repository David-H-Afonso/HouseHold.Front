...
import { DetailDrawer, ExternalProviderLink, FallbackImage, Icon, ModuleHeader, ModuleState, SearchBar } from '@/components/Shared'
...
export const GamesPage = () => {
	...
	const playWithNames = selectedGame?.playWithNames?.map((name) => name.trim()).filter(Boolean) ?? []
	return <div className='page-stack games-page'>
		<ModuleHeader title={<ExternalProviderLink provider='games-database'>Games</ExternalProviderLink>} />
		...
		{loading && games.length === 0 && <div className='games-page__skeleton-grid' aria-label='Loading games' aria-busy='true'>{Array.from({ length: 12 }, (_, index) => <div className='games-page__skeleton-card' key={index}><span className='games-page__skeleton-art' /><span className='games-page__skeleton-line games-page__skeleton-line--title' /><span className='games-page__skeleton-line' /></div>)}</div>}
		...
		<DetailDrawer open={selectedGame !== null} title={selectedGame?.name ?? 'Game detail'} onClose={() => setSelectedGame(null)}>{selectedGame && <div className='games-page__detail'>
			<div className='games-page__detail-hero'><FallbackImage src={selectedGame.hero ?? selectedGame.cover} alt={`${selectedGame.name} artwork`} fallbackLabel={selectedGame.name} width={900} height={330} />{selectedGame.favorite && <span className='games-page__detail-favorite'><Icon name='star' /> Favorite</span>}</div>
