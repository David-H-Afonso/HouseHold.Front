...
export const GamesPage = () => {
	...
	return <div className='page-stack games-page'>
		...
		<DetailDrawer open={selectedGame !== null} title={selectedGame?.name ?? 'Game detail'} onClose={() => setSelectedGame(null)}>{selectedGame && <div className='games-page__detail'>
			<div className='games-page__detail-cover'><FallbackImage src={selectedGame.cover} alt={`${selectedGame.name} cover`} fallbackLabel={selectedGame.name} width={240} height={360} />{selectedGame.favorite && <span><Icon name='star' /> Favorite</span>}</div>
			...
			<dl><div><dt>Status</dt><dd><span className='game-status-chip'>{selectedGame.statusName ?? 'Unknown'}</span></dd></div><div><dt>Platform</dt><dd>{selectedGame.platformName ?? 'Unknown'}</dd></div><div><dt>Started</dt><dd>{formatDate(selectedGame.started, preferences.timezone)}</dd></div><div><dt>Finished</dt><dd>{formatDate(selectedGame.finished, preferences.timezone)}</dd></div><div><dt>Playtime</dt><dd>{selectedGame.steamPlaytimeForever ? `${Math.round(selectedGame.steamPlaytimeForever / 60)} hours` : 'Not available'}</dd></div></dl>
