...
const WidgetShell = ({ id, title, provider, link, children, state, size }: { id: DashboardWidgetId; title?: string; provider: string; link: string; children: ReactNode; state: WidgetState<unknown>; size: string }) => {
	const label = title ?? provider
	return <section className={`operational-widget operational-widget--${id} is-${size}`}>
		<header><div><span>{provider}</span>{title && <h2>{title}</h2>}</div><Link to={link} aria-label={`Open ${label}`}><Icon name='external' /></Link></header>
