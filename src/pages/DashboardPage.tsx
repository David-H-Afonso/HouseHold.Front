...
const calendarDateRange = () => ({ from: new Date().toISOString(), to: new Date(Date.now() + 31 * 24 * 60 * 60 * 1000).toISOString() })
const calendarEventTime = (event: CalendarEvent, timeZone: string) => event.isAllDay ? 'All day' : new Intl.DateTimeFormat(undefined, { dateStyle: 'medium', timeStyle: 'short', timeZone }).format(new Date(event.startAtUtc))
...
const WidgetShell = ({ id, title, provider, providerUrl, link, children, state, size }: { id: DashboardWidgetId; title?: string; provider: string; providerUrl?: string | null; link: string; children: ReactNode; state: WidgetState<unknown>; size: string }) => {
	...
	return <section className={`operational-widget operational-widget--${id} is-${size}`}>
		<header className={title ? 'has-title' : 'without-title'}><div>{providerUrl ? <a className='provider-link' href={providerUrl} target='_blank' rel='noopener noreferrer'>{provider}</a> : <span>{provider}</span>}{title && <h2>{title}</h2>}</div><Link to={link} aria-label={`Open ${label}`}><Icon name='external' /></Link></header>
...
const DashboardWidgetSlot = ({ widget, children }: { widget: DashboardWidgetPreference; children: ReactNode }) => {
	return <div className={`dashboard-widget-slot dashboard-widget-slot--${widget.id} is-${widget.size}`}>
		{children}
	</div>
}
...
export const DashboardPage = () => {
	...
	const [calendar, setCalendar] = useState<WidgetState<CalendarEvent[]>>(pendingState)
	...
	useEffect(() => {
		...
		setGames(pendingState()); setMedia(pendingState()); setWarcraft(pendingState()); setApps(pendingState()); setPokemon(pendingState()); setJellyfin(pendingState()); setWorkflows(pendingState()); setCalendar(pendingState())
		...
		else void settle(moduleService.pokemon({ search: '', tagIds: preferences.pokemonDashboardMode === 'tag' && preferences.pokemonDashboardTagId ? [preferences.pokemonDashboardTagId] : [], skip: 0, take: 12, spriteSource: preferences.pokemonSpriteSource, favorite: preferences.pokemonDashboardMode === 'favorites' ? true : undefined }).then((result) => preferences.pokemonDashboardMode === 'recent' ? [...result.items].sort((left, right) => new Date(right.addedAt ?? 0).getTime() - new Date(left.addedAt ?? 0).getTime()) : result.items), setPokemon)
		void settle(operationsService.jellyfin(), setJellyfin)
		void settle(operationsService.workflows(), setWorkflows)
		const calendarRange = calendarDateRange()
		void settle(moduleService.calendarEvents(calendarRange.from, calendarRange.to), setCalendar)
		return () => { active = false }
	...
	const viewLimit = (id: DashboardWidgetId) => {
		const size = widgetSize(id)
		if (size === 'compact') return 1
		if (size === 'wide') return id === 'pokemon' ? 12 : 4
		return id === 'pokemon' ? 8 : 2
	}
	const jellyfinItems = jellyfin.data?.continueWatching.length ? jellyfin.data.continueWatching : jellyfin.data?.nextUp.slice(0, 3) ?? []
	const jellyfinPreview = [...new Map(jellyfinItems.map((item) => [item.seriesName ?? item.name, item])).values()].slice(0, viewLimit('jellyfin'))
	const jellywatchPreview = [...new Map((media.data ?? []).map((item) => [item.seriesId, item])).values()].slice(0, viewLimit('jellywatch'))
	...
	const content: Record<DashboardWidgetId, ReactNode> = {
		...
		games: <WidgetShell id='games' provider='Games Database' providerUrl={providerUrl('Games Database')} link='/games' state={games} size={widgetSize('games')}>{games.data?.length ? <HorizontalScroller label='Dashboard games'><div className='dashboard-carousel dashboard-carousel--games'>{games.data.slice(0, widgetSize('games') === 'wide' ? 7 : 5).map((game) => <article key={game.id} className='dashboard-game-card games-cover-card'><div className='games-cover-card__art'><FallbackImage src={game.cover} alt={`${game.name} cover`} fallbackLabel={game.name} width={240} height={360} /></div><div className='games-cover-card__meta'><strong title={game.name}>{game.name}</strong><span>{game.platformName ?? 'Platform not set'}</span></div></article>)}</div></HorizontalScroller> : <p className='widget-empty'>{preferences.gameStatusIds.length ? 'No games match the selected statuses.' : 'Choose game statuses in Dashboard Settings.'}</p>}</WidgetShell>,
		today: <WidgetShell id='today' title="Today's tasks" provider='DoIt' providerUrl={providerUrl('DoIt')} link='/today' state={{ loading: today.loading && !today.data, data: today.data, error: today.providerError }} size={widgetSize('today')}>{today.data && <><div className='dashboard-progress'><strong>{today.data.progress.done}/{today.data.progress.total}</strong><span>resolved today</span><i><b style={{ width: `${today.data.progress.total ? (today.data.progress.done / today.data.progress.total) * 100 : 0}%` }} /></i></div><div className='dashboard-task-list'>{today.data.tasks.slice(0, widgetSize('today') === 'wide' ? 7 : 5).map((task) => <TodayTaskActionRow key={task.occurrenceId} compact task={task} displayTimeZone={preferences.timezone} pending={today.pendingOccurrences.has(task.occurrenceId)} onAction={today.runAction} />)}</div></>}</WidgetShell>,
		calendar: <WidgetShell id='calendar' title='Upcoming events' provider='DoIt' providerUrl={providerUrl('DoIt')} link='/calendar' state={calendar} size={orderedWidgets.find((widget) => widget.id === 'calendar')?.size ?? 'medium'}>{calendar.data?.length ? <div className='dashboard-calendar-list'>{calendar.data.slice(0, 5).map((event) => <article key={event.id}><time>{calendarEventTime(event, preferences.timezone)}</time><strong>{event.title}</strong>{event.zoneName && <span>{event.zoneName}</span>}</article>)}</div> : <p className='widget-empty'>No upcoming events in the next 31 days.</p>}</WidgetShell>,
		jellywatch: <WidgetShell id='jellywatch' title='Next 7 days' provider='Jellywatch' providerUrl={providerUrl('Jellywatch')} link='/media' state={media} size={widgetSize('jellywatch')}>{jellywatchPreview.length ? <HorizontalScroller label='Upcoming Jellywatch episodes'><div className='dashboard-carousel dashboard-carousel--media'>{jellywatchPreview.map((item) => <article key={`${item.mediaItemId}-${item.episodeNumber}`}><FallbackImage src={item.posterUrl} alt={`${item.seriesTitle} poster`} fallbackLabel={item.seriesTitle} /><strong>{item.seriesTitle}</strong><span>S{item.seasonNumber} E{item.episodeNumber}</span><time>{formatUpcomingDate(item, preferences.timezone)}</time></article>)}</div></HorizontalScroller> : <p className='widget-empty'>Nothing is scheduled in the provider's 7-day window.</p>}</WidgetShell>,
		jellyfin: <WidgetShell id='jellyfin' title={jellyfin.data?.continueWatching.length ? 'Continue Watching' : 'Next Up'} provider='Jellyfin' providerUrl={providerUrl('Jellyfin')} link='/jellyfin' state={jellyfin} size={widgetSize('jellyfin')}>{jellyfinPreview.length ? <HorizontalScroller label='Jellyfin dashboard media'><div className='dashboard-carousel dashboard-carousel--jellyfin'>{jellyfinPreview.map((item) => { const url = safeExternalUrl(item.openUrl); const card = <><FallbackImage src={item.imageUrl} alt={`${item.name} artwork`} fallbackLabel={item.seriesName ?? item.name} /><strong>{item.seriesName ?? item.name}</strong><span>{item.name}</span></>; return url ? <a key={item.id} href={url} target='_blank' rel='noopener noreferrer'>{card}</a> : <article key={item.id}>{card}</article> })}</div></HorizontalScroller> : <p className='widget-empty'>No Continue Watching or Next Up items.</p>}</WidgetShell>,
		...
		pokemon: <WidgetShell id='pokemon' provider='Beast Vault' providerUrl={providerUrl('Beast Vault')} link='/pokemon' state={pokemon} size={widgetSize('pokemon')}>{preferences.pokemonDashboardMode === 'tag' && !preferences.pokemonDashboardTagId ? <p className='widget-empty'>Choose a Pokémon tag in Dashboard Settings.</p> : pokemon.data?.length ? <HorizontalScroller label='Dashboard Pokémon'><div className='dashboard-carousel dashboard-carousel--pokemon'>{pokemon.data.slice(0, widgetSize('pokemon') === 'wide' ? 12 : 8).map((item) => <Link to='/pokemon' key={item.id}><FallbackImage src={item.spriteUrl} fallbackSrc={item.fallbackSpriteUrl} alt={item.nickname ?? item.speciesName} fallbackLabel={item.speciesName} /><strong>{item.nickname ?? item.speciesName}</strong><span>Lv. {item.level}</span></Link>)}</div></HorizontalScroller> : <p className='widget-empty'>No Pokémon match this dashboard view.</p>}</WidgetShell>,
