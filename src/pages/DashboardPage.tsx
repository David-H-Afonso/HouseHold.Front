...
export const DashboardPage = () => {
	...
	const content: Record<DashboardWidgetId, ReactNode> = {
		...
		games: <WidgetShell id='games' provider='Games Database' providerUrl={providerUrl('Games Database')} link='/games' state={games} size={orderedWidgets.find((widget) => widget.id === 'games')?.size ?? 'medium'}>{games.data?.length ? <HorizontalScroller label='Dashboard games'><div className='dashboard-carousel dashboard-carousel--games'>{games.data.map((game) => <article key={game.id} className='dashboard-game-card games-cover-card'><div className='games-cover-card__art'><FallbackImage src={game.cover} alt={`${game.name} cover`} fallbackLabel={game.name} width={240} height={360} />{game.favorite && <span className='games-cover-card__favorite' title='Favorite'><Icon name='star' /></span>}<span className='games-cover-card__status' style={{ '--status-color': statuses.find((status) => status.id === game.statusId)?.color || '#64748b' } as CSSProperties}>{game.statusName ?? 'No status'}</span></div><div className='games-cover-card__meta'><strong title={game.name}>{game.name}</strong><span>{game.platformName ?? 'Platform not set'}</span></div></article>)}</div></HorizontalScroller> : <p className='widget-empty'>{preferences.gameStatusIds.length ? 'No games match the selected statuses.' : 'Choose game statuses in Dashboard Settings.'}</p>}</WidgetShell>,
