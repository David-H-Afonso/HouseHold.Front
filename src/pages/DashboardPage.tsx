...
const WidgetShell = ({ id, title, accessibleLabel, provider, providerId, providerUrl, link, children, state, size }: { id: DashboardWidgetId; title?: string; accessibleLabel?: string; provider: string; providerId?: HouseholdProviderId; providerUrl?: string | null; link: string; children: ReactNode; state: WidgetState<unknown>; size: string }) => {
	const label = title ?? provider
	return <section className={`operational-widget operational-widget--${id} is-${size}`} aria-label={accessibleLabel ?? label}>
		<header className={title ? 'has-title' : 'without-title'}><div><ExternalProviderLink className='provider-link' provider={providerId} href={providerUrl}>{provider}</ExternalProviderLink>{title && <h2>{title}</h2>}</div><Link to={link} aria-label={`View ${accessibleLabel ?? label} in Household`}><Icon name='chevronRight' /></Link></header>
