...
const completedLabel = (task: TodayTask) => {
	if (!task.completedAt) return null
	const date = new Date(task.completedAt)
	return Number.isNaN(date.getTime()) ? null : `Completed ${new Intl.DateTimeFormat(undefined, { hour: '2-digit', minute: '2-digit', second: '2-digit', hourCycle: 'h23', timeZone: task.timeZoneId }).format(date)}`
}
	...
	return <>
		<div className={`today-action-row${compact ? ' today-action-row--compact' : ''}${completed ? ' is-completed' : ''}${pending ? ' is-pending' : ''}`}>
			...
			<span className='today-action-row__content'>
				...
				<small>{[timeLabel(task), completedLabel(task)].filter(Boolean).join(' · ')}</small>
			</span>
		...
		<DetailDrawer open={detailsOpen} title={task.title} onClose={() => setDetailsOpen(false)}>
			...
			<div><dt>Zone</dt><dd>{task.zoneName ?? 'No zone'}</dd></div><div><dt>Scope</dt><dd>{task.scope}</dd></div><div><dt>Date</dt><dd>{task.occurrenceDate}</dd></div><div><dt>Availability</dt><dd>{task.availableFromTime ? `From ${task.availableFromTime.slice(0, 5)}` : 'Any time'}{task.availableUntilTime ? ` until ${task.availableUntilTime.slice(0, 5)}` : ''}</dd></div><div><dt>Recommended</dt><dd>{task.recommendedTime?.slice(0, 5) ?? 'Not set'}</dd></div><div><dt>Recurrence</dt><dd>{task.recurrenceType || 'Managed by DoIt'}</dd></div><div><dt>Assignment</dt><dd>{task.assignmentMode || 'Not assigned'}{task.assigneeNames.length ? ` · ${task.assigneeNames.join(', ')}` : ''}</dd></div><div><dt>Timezone</dt><dd>{task.timeZoneId}</dd></div>{task.completedAt && <div><dt>Completed at</dt><dd>{new Intl.DateTimeFormat(undefined, { dateStyle: 'medium', timeStyle: 'medium', timeZone: task.timeZoneId }).format(new Date(task.completedAt))}</dd></div>}<div><dt>Status</dt><dd>{task.occurrenceStatus}</dd></div>
			</dl>
		</DetailDrawer>
	</>
}
