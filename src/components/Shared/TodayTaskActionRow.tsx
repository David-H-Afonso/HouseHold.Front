	...
	return <>
		...
		<DetailDrawer open={detailsOpen} title={task.title} onClose={() => setDetailsOpen(false)}>
			<dl className='task-detail-list'>
				<div><dt>Zone</dt><dd>{task.zoneName ?? 'No zone'}</dd></div><div><dt>Scope</dt><dd>{task.scope}</dd></div><div><dt>Date</dt><dd>{task.occurrenceDate}</dd></div><div><dt>Availability</dt><dd>{task.availableFromTime ? `From ${task.availableFromTime.slice(0, 5)}` : 'Any time'}{task.availableUntilTime ? ` until ${task.availableUntilTime.slice(0, 5)}` : ''}</dd></div><div><dt>Recommended</dt><dd>{task.recommendedTime?.slice(0, 5) ?? 'Not set'}</dd></div><div><dt>Recurrence</dt><dd>{task.recurrenceType || 'Managed by DoIt'}</dd></div><div><dt>Assignment</dt><dd>{task.assignmentMode || 'Not assigned'}{task.assigneeNames.length ? ` · ${task.assigneeNames.join(', ')}` : ''}</dd></div><div><dt>Timezone</dt><dd>{task.timeZoneId}</dd></div><div><dt>Status</dt><dd>{task.occurrenceStatus}</dd></div>
			</dl>
		</DetailDrawer>
	</>
}
